	...
	def input_file(self):
		...
		if self.file_import:
			...
			for line in file_data:
				...
				#find product.
				product = self.env['product.product'].search([('name', '=', line[1]),('default_code', '=', line[0])])
				
				if not product:
					raise UserError('Product ' + line[1]  + ' [' + line[0] + ']')
				...
				move_line = self.env['stock.move'].search([('picking_id', '=', self.id),('product_id', '=', product.id)])
				if not move_line:
